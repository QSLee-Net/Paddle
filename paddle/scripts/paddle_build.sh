...
set +x
        ...
        easy_failed_case_parallel_job=$(echo $output | cut -d ";" -f 2)
        tetrad_parallel_job=$(echo $output | cut -d ";" -f 3)
        two_parallel_job=$(echo $output | cut -d ";" -f 4)
        non_parallel_job=$(echo $output | cut -d ";" -f 5)
        while read -r line; do
            if [[ "$line" == "" ]]; then
                ...
                if [[ "$is_exclusive" != "" ]]; then
                    ...
                    elif [[ $(echo $easy_failed_case_parallel_job | grep -o "\^$testcase\\$") != "" ]]; then
        ...
        card_test "$single_card_tests_high_parallel" 1 22       # run cases the most each time with single GPU
        card_test "$single_card_tests_secondary_high_parallel" 1 18
